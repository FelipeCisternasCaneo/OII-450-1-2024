...
def graficar_datos(iteraciones, fitness, xpl, xpt, tiempo, mh, problem, corrida):
    ...
    plt.savefig(f'{DIR_GRAFICOS}Convergence_{mh}_USCP_{problem}_{corrida}.pdf')
    plt.close('all')
    ...
    plt.savefig(f'{DIR_GRAFICOS}Percentage_{mh}_USCP_{problem}_{corrida}.pdf')
    plt.close('all')
    ...
    plt.savefig(f'{DIR_GRAFICOS}Time_{mh}_USCP_{problem}_{corrida}.pdf')
    plt.close('all')

def graficar_boxplot_violin(instancia):
    direccion_datos = f'{DIR_RESULTADO}fitness_USCP_{instancia}.csv'
    ...
    plt.savefig(f'{DIR_BOXPLOT}boxplot_fitness_USCP_{instancia}.pdf')
    plt.close()
    ...
    plt.savefig(f'{DIR_VIOLIN}violinplot_fitness_USCP_{instancia}.pdf')
    plt.close()
...
def graficar_mejores_resultados(instancia, mhs_instances):
    for name in MHS_LIST:
        ...
        plt.legend()
        plt.savefig(f'{DIR_BEST}fitness_USCP_{instancia}.pdf')
        plt.close()
...
for instancia in instancias:
    ...
    archivoResumenFitness = open(f'{DIR_RESULTADO}resumen_fitness_USCP_{instancia[1]}.csv', 'w')
    archivoResumenTimes = open(f'{DIR_RESULTADO}resumen_times_USCP_{instancia[1]}.csv', 'w')
    archivoResumenPercentage = open(f'{DIR_RESULTADO}resumen_percentage_USCP_{instancia[1]}.csv', 'w')
    archivoFitness = open(f'{DIR_RESULTADO}fitness_USCP_{instancia[1]}.csv', 'w')
