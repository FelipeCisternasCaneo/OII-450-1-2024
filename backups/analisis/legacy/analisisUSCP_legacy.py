...
def graficar_datos(iteraciones, fitness, xpl, xpt, tiempo, mh, problem, corrida):
    ...
    ax.set_title(f'Convergence {mh} \n uscp{problem} - Run {corrida} - ()')
    ax.set_ylabel("Fitness")
    ax.set_xlabel("Iteration")
    plt.savefig(f'{DIR_GRAFICOS}Convergence_{mh}_USCP_{problem}_{corrida}_.pdf')
    plt.close('all')
    ...
    plt.savefig(f'{DIR_GRAFICOS}Percentage_{mh}_USCP_{problem}_{corrida}_.pdf')
    plt.close('all')
...
def graficar_boxplot_violin(instancia):
    direccion_datos = f'{DIR_RESULTADO}fitness_USCP_{instancia}_.csv'
    ...
    plt.savefig(f'{DIR_BOXPLOT}boxplot_fitness_USCP_{instancia}_.pdf')
    plt.close()
    ...
    plt.savefig(f'{DIR_VIOLIN}violinplot_fitness_USCP_{instancia}_.pdf')
    plt.close()
...
def graficar_mejores_resultados(instancia, mhs_instances):
    ...
    for name in MHS_LIST:
        ...
        plt.title(f'Best Fitness \n uscp{instancia} - Mejor: {mh_mejor_fitness}')
        plt.ylabel("Fitness")
        plt.xlabel("Iteration")
        plt.legend()
        plt.savefig(f'{DIR_BEST}fitness_USCP_{instancia}_.pdf')
        plt.close()

        # Gráfico de mejores tiempos
        plt.plot(range(len(mh.bestTime)), mh.bestTime, color='b', label='Best Time')
        plt.title(f'Best Time (s) \n uscp{instancia} - Mejor: {mh_mejor_tiempo}')
        plt.ylabel("Time (s)")
        plt.xlabel("Iteration")
        plt.legend()
        plt.savefig(f'{DIR_BEST}time_USCP_{instancia}_.pdf')
        plt.close()
...
for instancia in instancias:
    print(f"Procesando instancia: uscp{instancia[1]}")
    blob = bd.obtenerArchivos(instancia[1])
    
    #print (blob)

    if not blob:
        print(f"Advertencia: La instancia uscp'{instancia[1]}' no tiene experimentos asociados. Saltando esta instancia...")
        continue

    # Crear archivos de salida
    archivoResumenFitness = open(f'{DIR_RESULTADO}resumen_fitness_USCP_{instancia[1]}_.csv', 'w')
    archivoResumenTimes = open(f'{DIR_RESULTADO}resumen_times_USCP_{instancia[1]}_.csv', 'w')
    archivoResumenPercentage = open(f'{DIR_RESULTADO}resumen_percentage_USCP_{instancia[1]}_.csv', 'w')
    archivoFitness = open(f'{DIR_RESULTADO}fitness_USCP_{instancia[1]}_.csv', 'w')
