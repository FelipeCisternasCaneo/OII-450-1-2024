...
def procesar_archivos(instancia, blob, archivo_fitness, MHS_LIST = MHS_LIST):
    corrida = 1
...
for instancia in instancias:
    ...
    for binarizacion in lista_bin:
        for mh in MHS_LIST:
            print(f"Procesando bin: {binarizacion}, MH: {mh}")

            # Obtener parámetros desde la base de datos
            parametros = bd.obtener_parametros_experimento(instancia[1], mh, binarizacion)
            if not parametros:
                print(f"[INFO] No hay parámetros para {instancia[1]}-{mh}-{binarizacion}. Saltando...")
                continue

            print(f"[DEBUG] Parámetros obtenidos: {parametros['paramMH']}")
        
            # Crear archivos de salida
            archivoResumenFitness = open(f'{DIR_RESULTADO}resumen_fitness_SCP_{instancia[1]}_{bin}.csv', 'w')
            archivoResumenTimes = open(f'{DIR_RESULTADO}resumen_times_SCP_{instancia[1]}_{bin}.csv', 'w')
            archivoResumenPercentage = open(f'{DIR_RESULTADO}resumen_percentage_SCP_{instancia[1]}_{bin}.csv', 'w')
            archivoFitness = open(f'{DIR_RESULTADO}fitness_SCP_{instancia[1]}_{bin}.csv', 'w')
