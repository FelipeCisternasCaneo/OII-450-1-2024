...
def _procesar_archivos(problem_cfg, instancia_id, blob, archivo_fitness, mhs_instances, bin_actual=None):
    ...
    for item in blob:
        ...
        # Generar nombre temporal ÚNICO usando PID y UUID
        ...
        try:
            ...
            # Intentar eliminar solo si existe
        ...
        if os.path.exists(temp):
            try:
                ...
                print(f"[ADVERTENCIA] No se pudo eliminar '{temp}': {e}")
...
def _procesar_instancia_binarizacion(inst, binarizacion, cfg, uses_bin, title_prefix):
    """Procesa una combinación instancia-binarización de forma independiente."""
    instancia_id = inst[1]
    ...
    # Crear instancia local de BD (importante para threading)
    bd_local = BD()
    blob = bd_local.obtenerArchivos(instancia_id, **cfg["obtenerArchivos_kwargs"])
...
def analizar_problema(problem_name: str, n_jobs=-1):
    """
    Analiza un problema con paralelización.
    
    Args:
        problem_name: Nombre del problema ('BEN', 'SCP', 'USCP')
        n_jobs: Número de trabajos paralelos (-1 = usar todos los cores)
    """
    ...
    # Crear todas las combinaciones instancia-binarización
...
def main():
    """Ejecuta el análisis con paralelización."""
    # n_jobs=-1 usa todos los cores disponibles
    # n_jobs=4 usa 4 cores
    # n_jobs=1 ejecuta secuencialmente (para debugging)
