...
def _workspace_root() -> str:
    """Devuelve la raíz del proyecto asumiendo que este archivo vive en /src/util/ o /Util/."""
...
def _resolver_ruta_existente(ruta: str) -> str:
    ...
    el caso común de usar 'util/' cuando el directorio real es 'src/util/'.
    """
    if not ruta:
...
def verificar_y_crear_carpetas():
    """
    Verifica que las carpetas y subcarpetas dentro de 'outputs/results' estén creadas.
    Si no existen, las crea automáticamente.
    """
    
    root_dir = _workspace_root()

    base_dir = os.path.join(root_dir, "outputs", "results")
    ...
    os.makedirs(os.path.join(root_dir, "outputs", "logs", "SSH"), exist_ok=True)
