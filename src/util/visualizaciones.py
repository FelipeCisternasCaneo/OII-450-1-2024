...
def obtener_optimo(funcion):
    ...
    Returns:
        float: El óptimo global
    """
    if funcion in OPTIMA_CLASSICAL:
        return OPTIMA_CLASSICAL[funcion]
        
    if funcion in OPTIMA_CEC2017:
        return OPTIMA_CEC2017[funcion]
        
    # Compatibilidad con formato CEC2017_F
...
def graficar_trayectoria_dimensional(mh, funcion, trajectory, dim):
    ...
    os.makedirs('./outputs/results/Trayectorias', exist_ok=True)
    plt.savefig(f'./outputs/results/Trayectorias/{mh}_{funcion}_trajectory_dims.png', dpi=300, bbox_inches='tight')
    plt.close()
    print(f" Guardado: ./outputs/results/Trayectorias/{mh}_{funcion}_trajectory_dims.png")
...
def graficar_search_history_2d_completo(mh, funcion, all_positions, lb, ub, best_solution=None, dim=2):
    ...
    os.makedirs('./outputs/results/SearchHistory', exist_ok=True)
    plt.savefig(f'./outputs/results/SearchHistory/{mh}_{funcion}_search_history_complete.png', 
                dpi=300, bbox_inches='tight')
    plt.close()
    print(f" Guardado: ./outputs/results/SearchHistory/{mh}_{funcion}_search_history_complete.png")
...
def graficar_search_history_2d(mh, funcion, search_history, lb, ub):
    ...
    os.makedirs('./outputs/results/SearchHistory', exist_ok=True)
    plt.savefig(f'./outputs/results/SearchHistory/{mh}_{funcion}_search_history_snapshots.png', 
                dpi=300, bbox_inches='tight')
    plt.close()
    print(f" Guardado: ./outputs/results/SearchHistory/{mh}_{funcion}_search_history_snapshots.png")
...
def graficar_search_history_slice(mh, funcion, search_history, dim1, dim2, lb, ub, dim):
    ...
    os.makedirs('./outputs/results/SearchHistory', exist_ok=True)
    plt.savefig(f'./outputs/results/SearchHistory/{mh}_{funcion}_search_slice_{dim1}_{dim2}.png', 
                dpi=300, bbox_inches='tight')
    plt.close()
    print(f" Guardado: ./outputs/results/SearchHistory/{mh}_{funcion}_search_slice_{dim1}_{dim2}.png")
