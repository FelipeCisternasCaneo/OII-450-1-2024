...
def fitness(problem, individual):
    ...
    if isinstance(individual, np.ndarray) and len(individual.shape) == 2:
        raise ValueError(
            "La función 'fitness' solo admite un individuo, no una población completa. Asegúrate de aplicar correctamente."
        )
    ...
    if isinstance(problem, str):
        if problem in BD.data:
            ...
            if problem in cec2017_functions:
                ...
                try:
                    fitness_value = classical_functions[problem](individual)
                except KeyError:
                    raise ValueError(
                        f"La función '{problem}' no está registrada en las funciones clásicas de benchmark."
                    )
        else:
            raise ValueError(f"El problema '{problem}' no es reconocido.")
    else:
        raise TypeError(
            "El parámetro 'problem' debe ser un string que identifique la función objetivo."
        )
...
def F3(
    x,
):  # Schwefel's Function No.1.2 (Double-Sum or Rotated Hyper-Ellipsoid Function) (CEC 2005 F2)
    dim = len(x) + 1
    o = 0
...
def F21(L):  # Shekel’s Function (Variant No. 5)
    ...
    for i in range(5):
        v = np.asarray(L - aSH[i, :])
        fit = fit - (np.dot(v, v) + cSH[i]) ** (-1)
...
# Registro seguro de funciones clásicas de benchmark (despacho sin globals)
classical_functions = {
