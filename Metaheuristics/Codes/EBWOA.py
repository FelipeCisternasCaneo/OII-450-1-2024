...
def iterarEBWOA(maxIter, t, dimension, population, bestSolution, lb, ub):
    ...
    kk = (1 - 0.5 * t / maxIter) * np.random.rand(population.shape[0])
        ...
        for j in range(dimension):
            ...
            if kk[i] > 0.5:  # Exploration phase
                ...
                if dimension <= population.shape[0] / 5:
                    params = np.random.permutation(dimension)[:2]  # Selección de 2 parámetros aleatorios
                    ...
                    params = np.random.permutation(dimension)  # Selección de todos los parámetros de forma aleatoria
                ...
                r3 = np.random.rand() - (2 * np.sin((np.pi / 2) * (t / maxIter) ** 4))
                r4 = np.random.rand()
                # Cálculo de C1
                C1 = 2 * r4 * (1 - t / maxIter)
                ...
                u = np.random.randn(1, dimension) * sigma  # Distribución normal
                v = np.random.randn(1, dimension)          # Distribución normal
                S = u / np.abs(v) ** (1 / alpha)
                # Parámetro de escala KD
                KD = 0.05
                LevyFlight = KD * S
                # Actualizar la posición del beluga i-ésimo
                population[i, j] = r3 * bestSolution[j] - r4 * population[i, j] + C1 * LevyFlight[0, j] * (population[RJ, j] - population[i, j])
    ...
    for i in range(population.shape[0]):
        for j in range(dimension):
            if kk[i] <= WF:
                ...
                stepsize2 = r7 * (ub - lb) * np.exp(-C2 * t / maxIter)
