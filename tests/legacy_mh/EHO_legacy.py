...
def iterarEHO(maxIter, iter, dim, population, best, lb, ub, fitness, fo):
    ...
    # Asegurarse de que lb y ub sean indexables
    if not isinstance(lb, (list, np.ndarray)): lb = [lb] * dim
    if not isinstance(ub, (list, np.ndarray)): ub = [ub] * dim
    ...
    for i in range(pop_size):
        ...
        # Harem (female) index
            ...
            new_individual = []
            for j in range(dim):
                gamma_val = np.random.uniform(-2, 2)
                beta = 1
                new_val = individual[j] + beta * ((population[MaleIndex, j] - individual[j])) + gamma_val * ((population[random_bull, j] - individual[j]))
